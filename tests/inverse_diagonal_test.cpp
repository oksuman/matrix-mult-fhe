...
namespace {
    ...
    template<typename T>
    std::vector<std::vector<double>> extractDiagonalVectors(const std::vector<double>& packedMatrix, T size) {
        const size_t d = static_cast<size_t>(size);
        std::vector<std::vector<double>> diagonalVectors(d, std::vector<double>(d, 0.0));
    ...
    template<typename T>
    std::vector<double> packDiagonalVectors(const std::vector<std::vector<double>>& diagonalVectors, T size) {
        const size_t d = static_cast<size_t>(size);
...
template <int d>
class MatrixInvDiagTest : public ::testing::Test {
protected:
    void SetUp() override {
        ...
        matInv = std::make_unique<MatrixInv_diag<d>>(m_enc, m_cc, m_publicKey, rotations, r);
    }
...
REGISTER_TYPED_TEST_SUITE_P(MatrixInvDiagTestFixture,
    MultTest,
    InverseTest);
...
using InvTestSizes = ::testing::Types<
    std::integral_constant<size_t, 4>,
    std::integral_constant<size_t, 8>,
    std::integral_constant<size_t, 16>
>;
