...
template <int d> class MatrixInverseRT22TestFixture : public ::testing::Test {
  protected:
    ...
    std::unique_ptr<MatrixInverse_RT22<d>> matInv;
    int r;
};

template <typename T>
class MatrixInverseRT22TestTyped
    : public MatrixInverseRT22TestFixture<T::value> {};

TYPED_TEST_SUITE_P(MatrixInverseRT22TestTyped);

TYPED_TEST_P(MatrixInverseRT22TestTyped, ComprehensiveInverseTest) {
    ...
    std::cout << "All tests completed for RT22 " << d << "x" << d << " matrix\n"
              << std::endl;
}

REGISTER_TYPED_TEST_SUITE_P(MatrixInverseRT22TestTyped,
...
INSTANTIATE_TYPED_TEST_SUITE_P(MatrixInverseRT22, MatrixInverseRT22TestTyped,
                               InverseTestSizes);